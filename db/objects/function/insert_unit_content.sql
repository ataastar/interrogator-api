    ...
    with json_data as (
        select * from json_populate_record(
         ...
         uc as (insert into unit_content (unit_tree_id, translation_link_id)
           select json_data.id, translation_link_id
           from json_data,
                tl
           returning *),
         phraseFrom as (insert into phrase (language_id, text)
           select l.to_lang_id, json_array_elements_text("from"::json)
           from json_data,
                languages l
           returning *),
         phraseTo as (insert into phrase (language_id, text)
           select l.from_lang_id, json_array_elements_text("to"::json)
           from json_data,
                languages l
           returning *),
         tFrom as (insert into translation (translation_link_id, phrase_id)
           select tl.translation_link_id, phraseFrom.phrase_id
           from tl,
                phraseFrom
           returning *),
         tTo as (insert into translation (translation_link_id, phrase_id)
           select tl.translation_link_id, phraseTo.phrase_id
           from tl,
                phraseTo)
