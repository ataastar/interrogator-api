...
BEGIN
  IF NOT p_answer_is_right THEN
    UPDATE translation_link SET next_interrogation_date = p_answer_time WHERE translation_link_id = p_translation_link_id;
    RETURN;
  END IF;

  SELECT MIN(a.answer_time) INTO v_first_right_answer_time
  FROM answer a
  ...
  UPDATE translation_link SET next_interrogation_date =
    CASE
      WHEN next_interrogation_date > p_answer_time THEN add_time(next_interrogation_date, v_next_interrogation_interval)
    ELSE
